 ...
 * @returns {T} The processed input, transformed and validated as per the provided functions.
 *
 * @example
 * // Retrieve a numeric input, converting from string to number
 * const numericInput = getInputWrapper({
 *   inputName: 'myNumericInput',
 *   transformInput: (input) => parseInt(input, 10),
 * });
 *
 * @example
 * // Retrieve and validate an input
 * const validatedInput = getInputWrapper({
 *   inputName: 'myInput',
 *   validator: (input) => {
 ...
 * });
 */
export const getInputWrapper = <T = string>({
  inputName,
  errorLogMessage = `Something went wrong while trying to retrieve the input: '${inputName}'`,
  inputOptions,
  transformInput = (input: string) => input as unknown as T,
  validator = () => {},
}: GetInputWrapperProps<T>): T => {
  try {
    const input = transformInput(getInput(inputName, inputOptions));
    validator(input);
    return input;
  } catch (error) {
